...
import { LogOut, HelpCircle, ThumbsUp, ThumbsDown, X, Loader2, RefreshCw, Eye, Trash2, Mail, Calendar, User, Tag } from 'lucide-react';
import AdminWhatIfPreview from './AdminWhatIfPreview';
...
const CATEGORY_LABELS = {
  ...
  'spacetime': 'Spacetime',
  'anomalies': 'Anomalies',
  'flightmysteries': 'Flight Mysteries',
  'limits': 'Limits',
  'timetravel': 'Time Travel',
  'aliencontact': 'Alien Contact',
  'physicsbreaking': 'Physics Breaking',
...
export function WhatIf({ onLogout }) {
  ...
  const fetchTheories = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch(`${API}/admin/whatif`);
      if (response.ok) {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const response = await fetch(`${API}/admin/whatif/${id}/approve`, {
        method: 'PATCH'
      });
      if (response.ok) {
        fetchTheories();
  ...
  const handleRejectSubmit = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await fetch(`${API}/admin/whatif/${selectedTheory._id}/reject`, {
        method: 'PATCH',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API}/admin/whatif/${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
        fetchTheories();
      ...
      <div style={{ maxWidth: 980, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
          {[
            ...
            <div key={stat.label} style={{ background: 'rgba(0, 11, 73, 0.6)', border: `1px solid ${stat.borderColor}`, padding: 20, borderRadius: 14, textAlign: 'center', backdropFilter: 'blur(10px)', transition: 'all 0.3s ease' }}>
              ...
              <div style={{ fontSize: 13, color: '#bfc3c6', marginTop: 4 }}>{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          ...
          filteredTheories.map(theory => (
            <div key={theory._id} style={{ background: 'rgba(0, 11, 73, 0.6)', border: '1px solid rgba(245, 158, 11, 0.2)', borderRadius: 16, padding: 24, marginBottom: 16, backdropFilter: 'blur(10px)', overflow: 'hidden', transition: 'all 0.3s ease' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 20 }}>
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <h3 style={{ margin: '0 0 10px', fontSize: 18, color: '#fff', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{theory.title}</h3>
                  <p style={{ margin: '0 0 14px', color: '#94a3b8', fontSize: 14, lineHeight: 1.6, maxHeight: '4.8em', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
                    {theory.description.substring(0, 200)}{theory.description.length > 200 ? '...' : ''}
                  </p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, fontSize: 13, color: '#64748b' }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <Tag size={14} color="#a78bfa" /> <span style={{ color: '#a78bfa', fontWeight: 500 }}>{CATEGORY_LABELS[theory.category] || theory.category}</span>
                    </span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {/* View Modal - Full Article Preview */}
